...
(async function(uid) {
  ...
  batch.set(user, createUserDto({
    uid,
    name: userInfo.displayName,
    email: userInfo.email,
  }));
  batch.set(menu, createMenuDto({
    id: menu.id,
    uid,
    name: 'Menu #1',
    contents: {
  ...
  batch.set(sushiDinnerMeal, createMealDto({
  ...
  batch.set(sushiDish, createDishDto({
    id: sushiDish.id,
    uid,
    name: 'Sushi',
  ...
  batch.set(misoSoupDish, createDishDto({
    id: misoSoupDish.id,
    uid,
    name: 'Miso Soup',
    type: 'side',
  ...
  batch.set(pescatarianTag, createTagDto({
  ...
  batch.set(vegetarianTag, createTagDto({
    id: vegetarianTag.id,
    name: 'Vegetarian',
    uid,
...
function createDocRef(db: admin.firestore.Firestore, endpoint: Endpoint, id?: string) {
  if (id) {
